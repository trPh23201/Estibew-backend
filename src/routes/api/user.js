  ...
  try {
    ...
    query
      ...
      .modifyGraph("purchased", (builder) => {
        builder
          .select("created as purchasedDate")
          .withGraphFetched("belongToGame(selectMany)")
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      const obj = {
        [req.file ? "avatar" : Object.keys(data)[0]]: req.file
          ? req.file.path
          : data[Object.keys(data)[0]],
      };
      if (
        Object.keys(data)[0] === "username" &&
        data[Object.keys(data)[0]].length > 20
      ) {
      ...
      if (sendNoti) {
        ...
        req.file
          ...
          : (content = `You updated your ${
              Object.keys(data)[0]
            }.\nDetails: from ${req.user[Object.keys(data)[0]] || "none"} to ${
              data[Object.keys(data)[0]]
            }`);

        const notification = await Notification.query().insertAndFetch({
          sender: "System",
          title: `You just updated your ${
            req.file ? "avatar" : Object.keys(data)[0]
          }. Click to see detail...`,
          content,
          image: `${setting.SERVER_URL}/static/infomation.jpg`,
          userId: req.user.id,
        });
...
router.get("/download/:gameId", restrict, async (req, res) => {
  try {
    const query = Purchased.query().where({
    ...
    if (result.length > 0)
      res
        .status(200)
        .download(
          path.join(__dirname, "../../../public/installation", "game.exe")
        );
