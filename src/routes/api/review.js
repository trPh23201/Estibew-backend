...
router.post("/create", async function (req, res) {
  try {
    ...
    if (!comment || like === undefined || !gameId)
      throw new Error("Missing parameter");
    const query = Review.query().insertAndFetch({
      comment,
      gameId,
      like,
      userId: req.user.id,
    ...
    const notification = await Notification.query().insertAndFetch({
      sender: "System",
      title: `You comment in ${gameName} and you ${
        result.like ? "like" : "dont like"
      } it`,
      content: `Details:\n${result.comment}`,
      image: gameImg,
      userId: req.user.id,
...
router.delete("/:id/delete", async function (req, res) {
  ...
  try {
    ...
    const notification = await Notification.query().insertAndFetch({
      ...
      image: `${setting.SERVER_URL}/static/review.jpg`,
      userId: req.user.id,
    });
