...
router.post("/register", async (req, res) => {
  try {
    ...
    const emailExist = await User.query()
      .where({ email, provider: null, providerId: null })
      .first();
    if (emailExist) {
    ...
    jwt.verify(
      ...
      async function (error, decoded) {
        ...
        res
          .status(200)
          .send(
            "<h1>Account verified! You can use this account to sign in</h1>"
          );
      }
    );
  } catch (error) {
  ...
  try {
    ...
    if (user) {
      ...
      const notification = await Notification.query().insertAndFetch({
        ...
        image: `${setting.SERVER_URL}/static/key.jpg`,
        userId: user.id,
      });
...
router.post("/update_password", middleware.restrict, async (req, res) => {
  try {
    ...
    if (password !== confirm)
      throw new Error("Password and confirm doesn't match!");
    const user = await User.query().findOne({
      id: req.user.id,
      provider: null,
      providerId: null,
    ...
    if (req.headers["old-to-new"]) {
      if (!User.validPassword(oldPassword, user.password))
        throw new Error("Your current password isn't correct!");
    }
    if (user)
      await User.query()
