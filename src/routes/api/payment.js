  ...
  try {
    ...
    games.forEach((game) => {
      if (gameIds.includes(game.id))
        throw new Error(
          `You already purchased ${game.name}, please remove it in your cart!`
        );
    });
...
router.get("/verify", async function (req, res) {
  ...
  try {
    ...
    if (sessions.payment_status === "paid" && sessions.status === "complete") {
      const list = await stripe.checkout.sessions.listLineItems(id, {
      ...
      list.data.forEach((item) => {
        content += `${item.description} - ${(item.amount_total / 100).toFixed(
          2
        )}$\n`;
      ...
      const notification = await Notification.query().insertAndFetch({
        sender: "System",
        title: `Thank for your purchased, total is ${total.toFixed(
          2
        )}$. Download games in profile`,
        content,
        image: `${setting.SERVER_URL}/static/pay.jpg`,
        userId: req.user.id,
      });
